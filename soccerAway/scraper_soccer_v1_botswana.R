...
webElem <- remDr$findElement(using = 'class name', as.character(flag[[24]]))#SELECCIONO Y ABRO EL PAIS
...
###############NIVEL 2
botswanaIndex <- data.frame(link = character(),
                           equipo=character(),
                           genero=character(),
                           liga=character())
...
assign(paste0('botswana',competitionsNombre[x]),df )
botswanaIndex <- bind_rows(botswanaIndex,df)
}
botswanaIndex <- botswanaIndex[-c(17:26,43:52),]
#botswanaIndex <- bind_rows(argeliaNPFL, argeliaOther)

#botswanaIndex<- botswanaIndex[-c(93:102),]
write_csv(botswanaIndex, "botswanaIndex.csv")
# EL LOOP SE HIZO MAL Y SE MULTIPLICARON EQUIPOS Y TIENE EL NOMBRE DE LA LIGA MAL
botswanaIndex <- botswanaIndex %>% mutate(estadioNombre = "vacio")
botswanaIndex <- botswanaIndex %>% mutate(capacidad = "0")
botswanaIndex <- botswanaIndex %>% mutate(latitud = -99999)
botswanaIndex <- botswanaIndex %>% mutate(longitud = -99999)
estadioNombre <- list()
capacidad <- list()
longitud <- list()
latitud <- list()
...
botswanaIndex2 <- botswanaIndex
  for (x  in 18:32) {
    
  urlEstadio <- paste0("https://int.soccerway.com", botswanaIndex[x,1])   # CAMBIAR EL 1 por X
...
if (!is_empty(a)){
  ...
  if  (is.na(noestadio[1]) == TRUE) {
    ...
    latitud[x] <- 999999
    longitud[x] = 99999 
    print(paste0(x, as.character(botswanaIndex[x,2])))
    
    botswanaIndex2[x,5] <- "999"
    botswanaIndex2[x,6] <-  "999"
    botswanaIndex2[x,7] <- 9999
    botswanaIndex2[x,8] <- 9999
  ...
  botswanaIndex2[x,5] <- as.character(detallesNombre[2])
  botswanaIndex2[x,6] <- detallesNombre[6]
  botswanaIndex2[x,7] <- latitudA
  botswanaIndex2[x,8] <- longitudA
...
write_csv(botswanaIndex2,"botswanaIndex2.csv")

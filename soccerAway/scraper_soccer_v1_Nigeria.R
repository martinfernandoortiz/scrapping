...
###############NIVEL 2
nigeriaIndex <- data.frame(link = character(),
                           equipo=character(),
                           genero=character(),
                           liga=character())
...
assign(paste0('nigeria',competitionsNombre[x]),df )
nigeriaIndex <- bind_rows(argeliaIndex,df)
}
nigeriaIndex <- bind_rows(argeliaNPFL, argeliaOther)

nigeriaIndex<- nigeriaIndex[-c(93:102),]
write_csv(nigeriaIndex, "nigeriaIndex.csv")
# EL LOOP SE HIZO MAL Y SE MULTIPLICARON EQUIPOS Y TIENE EL NOMBRE DE LA LIGA MAL
nigeriaIndex <- nigeriaIndex %>% mutate(estadioNombre = "vacio")
nigeriaIndex <- nigeriaIndex %>% mutate(capacidad = "0")
nigeriaIndex <- nigeriaIndex %>% mutate(latitud = -99999)
nigeriaIndex <- nigeriaIndex %>% mutate(longitud = -99999)
estadioNombre <- list()
capacidad <- list()
longitud <- list()
latitud <- list()
...
nigeriaIndex2 <- nigeriaIndex
  for (x  in 51:62) {
    
  urlEstadio <- paste0("https://int.soccerway.com", nigeriaIndex[x,1])   # CAMBIAR EL 1 por X
...
if (!is_empty(a)){
  ...
  if  (is.na(noestadio[1]) == TRUE) {
    ...
    latitud[x] <- 999999
    longitud[x] = 99999 
    print(paste0(x, as.character(nigeriaIndex[x,2])))
    
    nigeriaIndex2[x,5] <- "999"
    nigeriaIndex2[x,6] <-  "999"
    nigeriaIndex2[x,7] <- 9999
    nigeriaIndex2[x,8] <- 9999
  ...
  nigeriaIndex2[x,5] <- as.character(detallesNombre[2])
  nigeriaIndex2[x,6] <- detallesNombre[6]
  nigeriaIndex2[x,7] <- latitudA
  nigeriaIndex2[x,8] <- longitudA
...
write_csv(argeliaIndex2,"argeliaIndex2.csv")

...
webElem <- remDr$findElement(using = 'class name', as.character(flag[[29]]))#SELECCIONO Y ABRO EL PAIS
...
###############NIVEL 2
burquinaIndex <- data.frame(link = character(),
                           equipo=character(),
                           genero=character(),
                           liga=character())
...
assign(paste0('burquina',competitionsNombre[x]),df )
burquinaIndex <- bind_rows(burquinaIndex,df)
}
burquinaIndex <- burquinaIndex[c(1:18,47:64),]
#burquinaIndex <- bind_rows(argeliaNPFL, argeliaOther)

#burquinaIndex<- burquinaIndex[-c(93:102),]
write_csv(burquinaIndex, "burquinaIndex.csv")
# EL LOOP SE HIZO MAL Y SE MULTIPLICARON EQUIPOS Y TIENE EL NOMBRE DE LA LIGA MAL
burquinaIndex <- burquinaIndex %>% mutate(estadioNombre = "vacio")
burquinaIndex <- burquinaIndex %>% mutate(capacidad = "0")
burquinaIndex <- burquinaIndex %>% mutate(latitud = -99999)
burquinaIndex <- burquinaIndex %>% mutate(longitud = -99999)
estadioNombre <- list()
capacidad <- list()
longitud <- list()
latitud <- list()
...
burquinaIndex2 <- burquinaIndex
  for (x  in 24:36) {
    
  urlEstadio <- paste0("https://int.soccerway.com", burquinaIndex[x,1])   # CAMBIAR EL 1 por X
...
if (!is_empty(a)){
  ...
  if  (is.na(noestadio[1]) == TRUE) {
    ...
    latitud[x] <- 999999
    longitud[x] = 99999 
    print(paste0(x, as.character(burquinaIndex[x,2])))
    
    burquinaIndex2[x,5] <- "999"
    burquinaIndex2[x,6] <-  "999"
    burquinaIndex2[x,7] <- 9999
    burquinaIndex2[x,8] <- 9999
  ...
  burquinaIndex2[x,5] <- as.character(detallesNombre[2])
  burquinaIndex2[x,6] <- detallesNombre[6]
  burquinaIndex2[x,7] <- latitudA
  burquinaIndex2[x,8] <- longitudA
...
write_csv(burquinaIndex2,"burquinaIndex2.csv")
